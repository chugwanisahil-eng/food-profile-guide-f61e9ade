...
const Login = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (mode === "signup") {
        // Register
        const res = await fetch("http://127.0.0.1:5000/api/profile/register", {
          method: "POST",
        ...
        // Sign in
        const res = await fetch("http://127.0.0.1:5000/api/profile/login", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
        ...
        const data = await res.json();
        if (res.ok) {
          // Merge user + profile into localStorage
          const stored = { ...data.user, ...(data.profile || {}) };
